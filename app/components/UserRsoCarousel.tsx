...
import { DbRso, GetRsos, GetUserRsos, State } from "./server/actions";
import alertCreation from "./AlertCreation";
import UserRsoCard from "./UserRsoCard";
...
export function RsoCarouselContent({handleStateChange} : any) {
  ...
  useEffect(() => {
    const fetchEvents = async () => {
      try {
        setIsLoading(true); 
        const rsoList: DbRso[] = await GetUserRsos();
        setEvents(rsoList);
      } catch (ex : any) {
    ...
    <section className="max-w-sm sm:max-w-screen-md lg:max-w-screen-lg 2xl:max-w-screen-2xl w-80 space-x-4 m-4 p-4 md:w-full carousel carousel-center items-center
          ...
          {events && events?.map((item, index) => (
            <div className="mx-4 justify-center" key={index}>
              <UserRsoCard event={item} setIsLoading={setIsLoading} handleStateChange={handleStateChange}/>
            </div>
          ))}

          {events === null && 
            <div className="text-center justify-center self-center">
              There are not RSOs to show    
            </div>
            }
          </section>
  )
...
export default function UserRsoCarousel() {
